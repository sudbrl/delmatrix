...
<style>
    ...
    .upload-box-title { color: #1A1A18; font-size: 17px; font-weight: 700;
                        margin-bottom: 6px; }
    ...
    .template-card-title { color: #0D47A1; font-size: 13px; font-weight: 700;
                           margin-bottom: 6px; }
    .template-card pre {
...
_defaults = {
    ...
    "generated": False, "upload_error": None, "filename": None,
}
...
def parse_template(file_bytes: bytes):
    ...
    Returns (prev: ndarray shape(5,), trans: ndarray shape(5,5)).
    ...
    for i, row in df.iterrows():
        if sum(1 for cell in row if norm(str(cell)) in grade_norms) >= 4:
            header_row_idx = i
            break

    if header_row_idx is None:
        raise ValueError(
            "Could not find a header row containing the grade names: "
            "Good, Watchlist, Substandard, Doubtful, Bad."
        )

    # Build column map
    col_map = {}
    for ci, cell in enumerate(df.iloc[header_row_idx]):
        n = norm(str(cell))
        if n in grade_norms:
            col_map[n] = ci
    ...
    if len(col_map) < 5:
        ...
        raise ValueError(
            f"Only {len(col_map)}/5 grades found in header. "
            f"Missing: {missing}"
        )
    ...
    for i in range(header_row_idx + 1, len(df)):
        row = df.iloc[i]
        fv = norm(str(row.iloc[0]))
        if fv in grade_norms:
            data_rows[fv] = row
...
def render_matrix_html(trans, prev):
    ...
    for ri in range(n):
        ...
        rows_html += (
            f'<tr><td class="row-hdr">{ICONS[ri]} {GRADES[ri]}</td>'
            f'{cells}</tr>'
        )
    total_cells = "".join(
    ...
    return (
        '<div class="matrix-summary"><table><thead><tr>'
        '<th style="text-align:left;padding-left:14px;">From / To</th>'
        f'{headers}<th>Opening (Row Sum)</th>'
        f'</tr></thead><tbody>{rows_html}'
        f'<tr><td class="totals-hdr">Closing (Col Sum)</td>'
        f'{total_cells}</tr></tbody></table></div>'
    )
...
with st.sidebar:
    ...
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
        ...
        <span style="color:#1A1A18;font-size:18px;font-weight:700;
                     font-family:'IBM Plex Sans',sans-serif;">NRB Matrix Tool</span>
    </div>
    """, unsafe_allow_html=True)
    st.markdown("---")
    ...
    export_dpi = st.select_slider("🖼️ Export DPI", [100, 150, 220, 300], value=220)
...
| Bad         | 20.6   | 0.4       | 0.1         | 0.3      | 247.8 | 269.2       |
    ...
    if not st.session_state.generated:
        ...
        if uploaded is not None:
            ...
            try:
                ...
                st.session_state.upload_error = None
                st.markdown(
                    f'<div class="parse-success">'
                    f'✅ Parsed successfully: <b>{uploaded.name}</b> — '
                    f'{N}x{N} matrix loaded, {int(prev.sum()):,} cr opening balance.'
                    f'</div>',
                    unsafe_allow_html=True,
                )
            except Exception as exc:
                ...
                st.session_state.matrix       = None
                st.markdown(
                    f'<div class="parse-error">'
                    f'❌ <b>Parse error:</b> {exc}'
                    f'</div>',
                    unsafe_allow_html=True,
                )
    ...
    if st.session_state.prev is not None and st.session_state.matrix is not None:
        ...
        with sc1:
            st.markdown(
                f'<div class="gh-card">'
                f'<div class="gh-card-title">Total Opening (Row Sum)</div>'
                f'<div class="gh-card-value gh-blue">{prev_arr.sum():,.1f}'
                f'<span style="font-size:12px;color:#7A7670;"> cr</span></div>'
                f'</div>',
                unsafe_allow_html=True,
            )
        with sc2:
            st.markdown(
                f'<div class="gh-card">'
                f'<div class="gh-card-title">Total Closing (Col Sum)</div>'
                f'<div class="gh-card-value gh-blue">{trans_arr.sum():,.1f}'
                f'<span style="font-size:12px;color:#7A7670;"> cr</span></div>'
                f'</div>',
                unsafe_allow_html=True,
            )
        with sc3:
            ...
            st.markdown(
                f'<div class="gh-card">'
                f'<div class="gh-card-title">Retained on Diagonal</div>'
                f'<div class="gh-card-value gh-blue">{retained:,.1f}'
                f'<span style="font-size:12px;color:#7A7670;"> cr</span></div>'
                f'<div style="font-size:12px;color:#1565C0;">{ret_pct:.1f}% of total</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
        with sc4:
            ...
            balanced = diff < 0.01
            label    = "✅ Balanced" if balanced else f"⚠️ Delta {diff:,.2f}"
            cls      = "gh-green" if balanced else "gh-amber"
            st.markdown(
                f'<div class="gh-card">'
                f'<div class="gh-card-title">Balance Check</div>'
                f'<div class="gh-card-value {cls}" style="font-size:18px;">{label}</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
        ...
        st.markdown(
            ...
            'Row sum = Opening balance | Column sum = Closing balance'
            '</div>',
            unsafe_allow_html=True,
        )
        ...
        with bc2:
            if not st.session_state.period:
                st.warning("Add a Period Label in the sidebar for better chart labeling.")
            if st.button("🚀 Generate Transition Matrix",
                         use_container_width=True, type="primary"):
                st.session_state.generated = True
                st.success("Matrix generated! Switch to the Heatmap tab.")
        ...
        <div class="upload-box">
            ...
            <div class="upload-box-sub">
                Supports .xlsx files with 5 NRB loan grade rows and columns.<br>
    ...
    if st.session_state.generated:
        st.markdown(
            ...
            '✅ Matrix already generated. Head to the Heatmap or Statistics tabs. '
            'Use Upload New File in the sidebar to reset.'
            '</div>',
            unsafe_allow_html=True,
        )
...
with tab_heatmap:
    if not st.session_state.generated:
        st.info("Upload a template in the Upload Template tab and click Generate.")
    else:
        prev_arr  = st.session_state.prev
        trans_arr = st.session_state.matrix
        ...
        kpi_rows = [
            ...
            (k2, "Retained",       stats["retained"],
             f'Retention {stats["retention_pct"]:.1f}%', "gh-blue"),
            (k3, "Upgraded",       stats["upgraded"],
             f'Upgrade {stats["upgrade_pct"]:.1f}%',     "gh-green"),
            (k4, "Downgraded",     stats["downgraded"],
             f'Downgrade {stats["downgrade_pct"]:.1f}%', "gh-red"),
        ]
        for col, title, val, sub, cls in kpi_rows:
            with col:
                st.markdown(
                    f'<div class="gh-card">'
                    f'<div class="gh-card-title">{title}</div>'
                    f'<div class="gh-card-value {cls}">{val:,.1f}</div>'
                    f'<div style="font-size:12px;margin-top:4px;" class="{cls}">{sub}</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
        ...
        for ci in range(N):
            with cc[ci]:
                ...
                st.markdown(
                    f'<div class="gh-card">'
                    f'<div class="gh-card-title">{ICONS[ci]} {GRADES[ci]}</div>'
                    f'<div class="gh-card-value" style="font-size:18px;">'
                    f'{col_closing[ci]:,.1f}'
                    f'<span style="font-size:11px;color:#7A7670;"> cr</span></div>'
                    f'<div style="font-size:11px;color:{d_color};">'
                    f'{delta_str} vs opening</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
...
2. The app auto-detects the header row and grade labels.
...
- A **header row** containing the 5 grade names:
  `Good`, `Watchlist`, `Substandard`, `Doubtful`, `Bad`
- **5 data rows** — each starting with its grade name
- Cell values = amount flowing from the row grade (opening) to column grade (closing)
- An optional **Grand Total** column (ignored by the parser)
""")

    st.markdown("### Matrix Logic")
    st.code(
...
""")
    ...
    st.markdown("---")
    st.markdown(
        '<div style="color:#9A9690;font-size:11px;font-family:'
        "'IBM Plex Mono',monospace;\">"
        "Built for Nepal Rastra Bank loan classification reporting."
        "</div>",
        unsafe_allow_html=True,
    )
